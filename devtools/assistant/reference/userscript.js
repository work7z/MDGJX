...
const read = "%s/read".replace("%s", source),
  preread = "%s/preread".replace("%s", source);
...
const regex = {
  ...
  lastReleaseRegex:
    /Last Release on (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s(\d{1,2}),\s(\d{4})/, // "Last Release on Jul 4, 2022"
  ...
  joinedRegex:
    /Joined ((Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec).*\s\d{1,2},?\s\d{4}$)/, // Joined March 27, 2022
  moreRegex: /\+(\d+) more\.\.\./, // More 100
  commentsRegex: /(\d+)\sComments/, // 数字 Comments
  gamesRegex: /(\d{1,3}(?:,\d{3})*)( games| Collections)/,
  combinedDateRegex:
    /^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec).*\s\d{1,2},?\s\d{4}$|^\d{1,2}\s(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec).*,?\s\d{4}$|^\d{1,2}\/\d{1,2}\/\d{4}$/i,
  emailRegex: /^Receive feedback emails \((.*)\)$/,
  verifyDomain:
    /To verify ownership of (.*), navigate to your DNS provider and add a TXT record with this value:/,
...
const localStorageManager = {
  ...
  clearLocalStorageIfNewSession() {
    ...
    if (
      ...
      currentTime - parseInt(lastSessionTimestamp) > 24 * 3600000
    ) {
...
const customGPT = {
  ...
  setGPTUrl(model, url) {
    ...
    let cloudflareReg =
      /https:\/\/gateway.ai.cloudflare.com\/v1\/\w+\/\w+\/openai\/chat\/completions/;
    if (
      url === this.openai ||
...
const settingManager = {
  ...
  <div style="font-size: 1em;" xmlns="http://www.w3.org/1999/html">
    <label class="instant-setting-label">快捷键<select id="fluent-read-hotkey" class="instant-setting-common">${this.generateOptions(
      shortcutManager.hotkeyOptions,
      util.getValue("hotkey"),
    )}</select></label>
    <label class="instant-setting-label">翻译源语言<select id="fluent-read-from" class="instant-setting-common">${this.generateOptions(
      langManager.from,
      langManager.getFrom(),
    )}</select></label>
    <label class="instant-setting-label">翻译目标语言<select id="fluent-read-to" class="instant-setting-common">${this.generateOptions(
      langManager.to,
      langManager.getTo(),
    )}</select></label>
    <label class="instant-setting-label">翻译服务<select id="fluent-read-model" class="instant-setting-select">${this.generateOptions(
      transModelName,
      util.getValue("model"),
    )}</select></label>
    ...
    <input type="text" class="instant-setting-input" id="fluent-read-custom-type" value="${optionsManager.getOption(
      util.getValue("model"),
    )}" ></label>
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if ([transModel.openai, transModel.ollama].includes(model)) {
          let ok = customGPT.setGPTUrl(
            model,
            util.getElementValue("fluent-read-custom"),
          );
          if (!ok) {
            toast.fire({
              icon: "error",
        ...
        if (model === transModel.ollama) {
          optionsManager.setOption(
            model,
            util.getElementValue("fluent-read-custom-type"),
          );
        } else {
          optionsManager.setOption(
            model,
            util.getElementValue("fluent-read-option"),
          );
        }
        // 5、存储 token
        ...
        // 6、设置 chatGPT 消息模板
        chatMgs.setSystemMsg(
          util.getElementValue("fluent-read-system-message"),
        );
    ...
    // 监听“翻译服务”选择框
    document
      .getElementById("fluent-read-model")
  ...
  setDisplayStyle(flex, none) {
    ...
    const customTypeLabel = document.getElementById(
      "fluent-read-custom-type-label",
    );
    ...
    customLabel.style.display = [transModel.openai, transModel.ollama].includes(
      model,
    )
      ? "flex"
      : "none"; // 判断是否显示自定义 GPT 地址输入框
    document.getElementById("fluent-read-custom").value =
      customGPT.getGPTUrl(model); // 设置自定义 GPT 地址
    ...
    if (model === transModel.ollama) {
      ...
      document.getElementById("fluent-read-custom-type").value =
        optionsManager.getCustomOption(model);
    } else {
      const optionSelect = document.getElementById("fluent-read-option");
      optionSelect.innerHTML = settingManager.generateOptions(
        optionsManager[model],
        optionsManager.getOption(model),
      );
  ...
  update() {
    // 跳转页面
    window.open(
      "https://greasyfork.org/zh-CN/scripts/482986-%E6%B5%81%E7%95%85%E9%98%85%E8%AF%BB",
    );
  },
  about() {
    // 跳转页面
...
(function () {
  ...
  window.addEventListener(
    "blur",
    () => (shortcutManager.hotkeyPressed = false),
  );
  ...
  checkRun((shouldRun) => {
    ...
    const observer = new MutationObserver(function (mutations, obs) {
      mutations.forEach((mutation) => {
        ...
        // 如果不包含下面节点，则处理
        if (
          !["img", "noscript"].includes(mutation.target.tagName.toLowerCase())
        ) {
    ...
    if (outerHTMLCache) {
      ...
      setTimeout(() => {
        ...
        if (fn) {
          ...
          node.outerHTML = safeFluentRead
            ? safeFluentRead.createHTML(outerHTMLCache)
            : outerHTMLCache;
        }
...
function getTransNode(node) {
  ...
  // 4、检测当前节点是否满足翻译条件
  if (
    getTransNodeSet.has(node.tagName.toLowerCase()) ||
...
function detectChildMeta(parent) {
  ...
  while (child) {
    if (
      ...
      !detectChildMetaSet.has(child.nodeName.toLowerCase())
    ) {
      return false;
    }
    child = child.nextSibling;
...
function translate(node) {
  ...
  // 检测语言类型，如果是中文则不翻译
  baiduDetectLang(node.innerText)
    .then((lang) => {
      if (lang === langManager.getTo()) return; // 与目标语言相同，不翻译

      // 如果是机器翻译，则翻译 outerHTML，否则递归获取文本
      let origin = isMachineTrans(model)
        ? node.outerHTML
        : getTextWithNode(node);

      let spinner = createLoadingSpinner(node); // 插入转圈动画
      let timeout = setTimeout(() => {
        createFailedTip(
          node,
          new Error(errorManager.netError).toString(),
          spinner,
        );
      }, 60000);

      // 调用翻译服务
      transModelFn[model](origin)
        .then((text) => {
          ...
          if (isMachineTrans(model)) {
            ...
            if (fn) {
              ...
              node.outerHTML = safeFluentRead
                ? safeFluentRead.createHTML(text)
                : text;
            }
          } else {
            // LLM 翻译
            node.innerHTML = safeFluentRead
              ? safeFluentRead.createHTML(text)
              : text;
            newOuterHtml = node.outerHTML;
          }
        ...
        .catch((e) => {
          ...
          createFailedTip(
            node,
            e.toString() || errorManager.unknownError,
            spinner,
          );
        });
    })
      ...
      .then((jwtString) => {
        ...
        ({
          ...
          url:
            ...
            "&to=" +
            langManager.getTo() +
            "&api-version=3.0&includeSentenceLength=true&textType=html",
        ...
        contents: [
          {
            role: "user",
            parts: [
              { text: chatMgs.getSystemMsg() + chatMgs.getUserMsg("hello") },
            ],
          },
          {
            role: "model",
...
function generateToken(apiKey) {
  ...
  // 存储
  tokenManager.setToken(transModel.zhipu, {
    apikey: apiKey,
    token: token,
...
function initApplication() {
  ...
  commonConfig.forEach((v) =>
    !util.getValue(v.name) ? util.setValue(v.name, v.value) : null,
  );
  ...
  GM_registerMenuCommand(
    `原始语言：${langManager.from[util.getValue("from")]}`,
    () => settingManager.setLanguage("from"),
  );
  GM_registerMenuCommand(
    `目标语言：${langManager.to[util.getValue("to")]}`,
    () => settingManager.setLanguage("to"),
  );
  GM_registerMenuCommand(
    `鼠标快捷键：${shortcutManager.hotkeyOptions[util.getValue("hotkey")]}`,
    () => settingManager.setHotkey(),
  );
  GM_registerMenuCommand(
    `翻译服务：${transModelName[util.getValue("model")]}`,
  ...
  skipStringMap[exceptionMap.openai_web] = function (node) {
    return (
      ...
      node.hasAttribute("data-projection-id")
    );
  };
  ...
  skipStringMap[exceptionMap.coze] = function (node) {
    return (
      node.classList.contains("auto-hide-last-sibling-br") ||
  ...
  switch (node.nodeType) {
    ...
    case Node.ELEMENT_NODE:
      ...
      if (node.hasAttribute("aria-label"))
        processNode(node, textType.ariaLabel, respMap);
      // 按钮与文本域节点
      if (
        ["input", "button", "textarea"].includes(node.tagName.toLowerCase())
      ) {
        ...
        if (
          ...
          processNode(node, textType.inputValue, respMap);
      }
      break;
    // 2、文本节点
    case Node.TEXT_NODE:
...
function processNode(node, attr, respMap) {
  ...
  if (formattedText && withoutChinese(formattedText)) {
    signature(url.host + formattedText).then((sign) =>
      respMap[sign] ? replaceText(attr, node, respMap[sign]) : null,
    );
  }
}
...
function procOpenai(node, respMap) {
  ...
  if (text && withoutChinese(text)) {
    ...
    if (dateOrFalse) {
      node.textContent = `${dateOrFalse.getFullYear()}-${
        dateOrFalse.getMonth() + 1
      }-${dateOrFalse.getDate()}`;
      return;
    }
...
function procMaven(node, respMap) {
  ...
  if (text && withoutChinese(text)) {
    ...
    if (repositoriesMatch) {
      ...
      node.textContent =
        repositoriesMatch[1] === "Repositories"
          ? `索引库数量（${count}）`
          : `索引包数量（${count * 100}万）`;
      return;
    }
    ...
    if (lastReleaseMatch) {
      let date = new Date(
        `${lastReleaseMatch[1]} ${lastReleaseMatch[2]}, ${lastReleaseMatch[3]}`,
      );
      node.textContent = `Last Release on ${date.getFullYear()}-${
        date.getMonth() + 1
      }-${date.getDate()}`;
      return;
    }
    ...
    let dateOrFalse = parseDateOrFalse(text);
    if (dateOrFalse) {
      node.textContent = `${dateOrFalse.getFullYear()}-${String(
        dateOrFalse.getMonth() + 1,
      ).padStart(2, "0")}-${String(dateOrFalse.getDate()).padStart(2, "0")}`;
      return;
    }
    // 处理依赖类型
...
function procDockerhub(node, respMap) {
  ...
  if (text && withoutChinese(text)) {
    ...
    if (joinedMatch) {
      ...
      node.textContent = `加入时间：${date.getFullYear()}-${String(
        date.getMonth() + 1,
