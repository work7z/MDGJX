...
const { session } = require("./remote-common");
const { createLogFileName } = require("../common/create-session-log-file-path");
const SessionLog = require("./session-log");
const { isWin } = require("../common/runtime-constants");
class Client {}
...
class Terminal {
  ...
  localInit(initOptions) {
    ...
    const exec = platform.startsWith("win")
      ? resolve(process.env.windir, execWindows)
      : platform === "darwin"
      ? execMac
      : execLinux;
  ...
  getShellWindow(initOptions = this.initOptions) {
    return _.pick(initOptions, ["rows", "cols", "term"]);
  }

  remoteInitTerminal(initOptions) {
    const connInst = session(initOptions.sessionId);
    const { conn, shellOpts } = connInst;
    if (initOptions.enableSsh === false) {
      return true;
    }
    return new Promise((resolve, reject) => {
      conn.shell(this.getShellWindow(), shellOpts, (err, channel) => {
        if (err) {
          return reject(err);
        }
  ...
  onKeyboardEvent(options) {
    ...
    this.ws.s({
      ...
      ..._.pick(this.initOptions, ["sessionId", "tabId"]),
      options,
    });
    return new Promise((resolve, reject) => {
      this.ws.once((arg) => {
      ...
      const run = (info) => {
        ...
        conn
          .on(
            ...
            async (name, instructions, instructionsLang, prompts, finish) => {
              ...
              const result = await this.onKeyboardEvent(options).catch(reject);
              finish(result);
            }
          )
          ...
          .on("ready", () => {
            ...
            conn.shell(shellWindow, shellOpts, (err, channel) => {
              if (err) {
                return reject(err);
              }
              this.channel = channel;
  ...
  onEndConn() {
    const inst = global.sessions[this.initOptions.sessionId];
    if (!inst) {
      return;
    }
    delete inst.sftps[this.pid];
    delete inst.terminals[this.pid];
    if (_.isEmpty(inst.sftps) && _.isEmpty(inst.terminals)) {
      inst.conn && inst.conn.end && inst.conn.end();
      delete global.sessions[this.initOptions.sessionId];
    }
  }
        ...
        resolve(
          list.map((item) => {
            const {
              filename,
              longname,
              attrs: { size, mtime, atime, uid, gid, mode },
            } = item;
...
exports.testConnection = (options) => {
  return new Terminal(options).remoteInit(options, true).catch(() => {
    return false;
  });
};
