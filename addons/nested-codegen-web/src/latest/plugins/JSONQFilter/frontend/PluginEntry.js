...
window.ExtensionDefinition["ROOT_EXTENSION_ADDONS"] = (gref) => {
  return {
    ...
    render: fn_otherPages.form.jsonHelperRender({
      helperText: `If you would like to filter some values in the value of an object or array type, you can use this function to apply what you need with ease. Meanwhile, please be noted that the function support using script only.`,
...
users = fn_data_filter(users, (value, index)=>{
  return index != 'c' // ${t(
...
users.a = fn_data_filter(users.a, (value, index, collection)=>{
  return !value.active; // when value.active != true, keep it
})

// ${t(`filtering the property "b" of the variable users`)}
users.b = fn_data_filter(users.b, (value, index, collection)=>{
  return value.user.indexOf('2') == -1; // when user has no '2' char, keep it
})
