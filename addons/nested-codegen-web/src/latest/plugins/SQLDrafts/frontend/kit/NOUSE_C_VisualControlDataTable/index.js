import C_NeedUpgradeFeature from "../C_NeedUpgradeFeature";
...
import "./color.less";
const Draggable = CodeGenDefinition.EXT_react_draggable.default;
const Divider = CodeGenDefinition.BluePrintCpt.Divider;
// const { DraggableCore } = Draggable
...
class VisualControlDataTable extends React.Component {
  ...
  isNotActiveNow = () => {
    return !this.state.enterScopeNow;
    // return this.props.notActive || this.props.notActive();
  };
  ...
  componentDidMount = () => {
    ...
    setInterval(() => {
      if (_.isEmpty(this.getExtra().calcColumnIndexArr)) {
        this.basic_refreshCalculateValueAndLineHeightAndScrollInfo();
      }
    }, 200);
    let n1 = reaction(
    ...
    gutils.keyDownListenObj["kd" + this.boxId] = (e) => {
      if (_.isNil(this.isNotActiveNow())) {
        // debugger;
        return;
      }
