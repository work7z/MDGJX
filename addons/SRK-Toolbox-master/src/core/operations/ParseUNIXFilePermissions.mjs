...
class ParseUNIXFilePermissions extends Operation {
    ...
    constructor() {
        ...
        this.name = "解析UNIX文件权限";
        this.module = "Default";
        this.description = "根据UNIX/Linux文件权限字符串（八进制或文本形式），解释具体什么权限赋给了谁。<br><br>输入必须为八进制（例如 <code>755</code>）或文本（例如 <code>drwxr-xr-x</code>）格式。";
    ...
    run(input, args) {
        ...
        if (input.search(/\s*[0-7]{1,4}\s*/i) === 0) {
            ...
            throw new OperationError("无效的输入格式。\n请输入八进制形式（例如：755）或文本形式（例如：drwxr-xr-x）。");
        }

        output += "文本形式： " + permsToStr(perms);
        output += "\n八进制形式： " + permsToOctal(perms);
 ...
 |         |  用户  |  组   |  其它  |
 +---------+-------+-------+-------+
 |    读   |   ${perms.ru ? "X" : " "}   |   ${perms.rg ? "X" : " "}   |   ${perms.ro ? "X" : " "}   |
 +---------+-------+-------+-------+
 |    写   |   ${perms.wu ? "X" : " "}   |   ${perms.wg ? "X" : " "}   |   ${perms.wo ? "X" : " "}   |
 +---------+-------+-------+-------+
